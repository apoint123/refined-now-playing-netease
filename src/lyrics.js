...
export function Lyrics(props) {
		...
		for (let i = 0; i < lyrics.length; i++) {
			...
			if (line.isInterlude) {
				if (allToNonInterlude.length > 0) {
					allToNonInterlude.push(
						allToNonInterlude[allToNonInterlude.length - 1],
					);
				} else {
	...
	useEffect(() => {
		...
		for (let i = current + 1; i < lyrics.length; i++) {
			...
			const previousScaledHeight =
				heightOfItems.current[i - 1] * transforms[i - 1].scale;
	...
	const onPlayProgress = (id, progress) => {
		...
		let startIndex = 0;
		if (
			currentTimeWithOffset - lastTime > 0 &&
			currentTimeWithOffset - lastTime < 50
		) {
	...
	useEffect(() => {
		...
		document.addEventListener(
			"rnp-karaoke-animation",
			onKaraokeAnimationChange,
		);
		document.addEventListener(
			"rnp-current-lyric-alignment-percentage",
			onCurrentLyricAlignmentPercentageChange,
		);
		...
		return () => {
			...
			document.removeEventListener(
				"rnp-rotate-curvature",
				onRotateCurvatureChange,
			);
			document.removeEventListener(
				"rnp-karaoke-animation",
				onKaraokeAnimationChange,
	...
	const karaokeAnimationFloat = (word) => {
		...
		if (
			...
			return {
				...
				transitionDelay: `0ms`,
				opacity: Math.max(
					0.4 + (0.6 * (props.currentTime - word.time)) / word.duration,
					0.4,
				),
...
function LyricOverview(props) {
	...
	return (
		<div
			...
			ref={props.overviewContainerRef}
		>
