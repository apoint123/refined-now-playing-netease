...
type ChangeTuple = [ChangeType, string, boolean?];
...
function WhatsNew(props: WhatsNewProps) {
	...
	const handleClose = () => {
		...
		const mask = model.parentElement;
		if (!mask) return;
		...
		const maskAnimation = mask.animate([{ opacity: 1 }, { opacity: 0 }], {
			duration: 300,
			easing: "ease",
		});
	...
	const renderIcon = (type: ChangeType) => {
		switch (type) {
			case "add":
				return (
		...
		<div className="rnp-whats-new" ref={modelRef}>
			...
			<div className="rnp-whats-new-content">
				{changeLog
					.filter(
						(version) =>
							compareVersions(version.version, props.lastVersion) > 0,
					)
					.sort((a, b) => compareVersions(b.version, a.version))
					.map((version) => (
						<div className="rnp-whats-new-version" key={version.version}>
							<h3>{version.version}</h3>
							<ul>
								{version.changes.map((change, index) => (
									<li key={index} className={`${change[2] ? "major" : ""}`}>
										{renderIcon(change[0])}
										{change[1]}
									</li>
								))}
			...
			<div className="rnp-whats-new-buttons">
				<button className="rnp-whats-new-button" onClick={handleClose}>
					关闭
				</button>
			</div>
		</div>
...
export function whatsNew(force = false) {
	let lastVersion =
		localStorage.getItem("refined-now-playing-last-version") ?? "0.0.0";
	...
	const hasNewContent =
		changeLog.filter(
			(version) => compareVersions(version.version, lastVersion) > 0,
		).length > 0;

	if (!hasNewContent && !force) return;

	const whatsNewContainer = document.createElement("div");
	whatsNewContainer.id = "refined-now-playing-whats-new";
	document.body.appendChild(whatsNewContainer);

	if (force) {
		lastVersion = "0.0.0";
	}

	ReactDOM.render(<WhatsNew lastVersion={lastVersion} />, whatsNewContainer);
}
