...
function Wizard() {
	...
	useEffect(() => {
		const checkGPU = async () => {
			...
			try {
				const configVal = await betterncm.app.readConfig(
					"cc.microblock.betterncm.remove-disable-gpu",
					"",
				);

				const isHardwareAccelDisabled = await new Promise<boolean>(
					(resolve) => {
						channel.call(
							"app.getLocalConfig",
				...
				if (configVal !== "true" && isHardwareAccelDisabled) {
	...
	useEffect(() => {
		const checkHijack = async () => {
			if (typeof betterncm.app.readConfig === "undefined") return;
			try {
				const val = await betterncm.app.readConfig(
			...
			<div className="rnp-compatibility-check__content">
				...
				{isNCMOutdated && (
					<>
						...
						<p className="warning">
							检测到您的网易云版本过旧，将会导致 Refined Now Playing
							无法正常工作。请更新网易云。
						</p>
						<Button
							text="下载新版网易云"
				...
				<h1>BetterNCM 版本</h1>
				<p>
					请尽可能将 BetterNCM 更新到最新版本，BetterNCM 版本过低会导致 Refined
					Now Playing 插件无法运行。
				</p>
				<p>
					目前推荐使用最新稳定版。如果版本过旧，请在 BetterNCM Installer
					中，点击 “重装/更新” 以更新最新版。
				</p>
				{isBetterNCMOutdated && (
					<p className="warning">
						检测到您的 BetterNCM 版本过旧，可能会导致 Refined Now Playing
						无法正常工作。请更新 BetterNCM。
					</p>
				)}
				{!isBetterNCMOutdated && (
					<p className="pass">
						检测到您的 BetterNCM 版本没有过旧。但如果仍然出现问题，请尝试更新
						BetterNCM。
					</p>
				)}
				<Button
					...
					onClick={async () => {
						await betterncm.app.exec(
							"https://github.com/MicroCBer/BetterNCM-Installer/releases",
						);
					}}
				/>
				<h1>GPU 加速</h1>
				<p>
					如果 GPU 加速被禁用，可能会导致：卡顿、模糊背景渲染错误、帧数低、CPU
					占用高等问题。
				</p>
				{isGPUDisabled ? (
					<p className="warning">
						检测到您的 GPU 加速已被禁用，可能会导致 Refined Now Playing
						无法正常工作。请启用 GPU 加速。
					</p>
				) : (
					<p className="pass">
						未检测到您的 GPU
						加速被禁用。但如果仍旧出现以上问题，请尝试使用以下的按钮启用 GPU
						加速。
					</p>
				)}
				<Button
				...
				{isHijackDisabled ? (
					<p className="warning">
						检测到您的 Hijack JS 注入已被禁用。请启用 Hijack JS 注入。
					</p>
				) : (
					<p className="pass">
						Hijack JS 注入未被禁用。但如果仍旧无法显示歌词，请点击以下 "清空
						Hijack 缓存按钮"。
					</p>
				)}
				{
				...
				<h1>性能</h1>
				{
					<>
						<p>
							Refined Now Playing 的某些效果依赖 GPU 渲染，如果设备 GPU
							性能较差，会造成低帧率、高占用等问题。
						</p>
						<p>
							如果已完成上述步骤，
							<b>
								但仍然出现性能问题，请尝试在播放页面右上角菜单中，检查以下选项：
							</b>
						</p>
						<ul>
							<li>
						...
						<p>
							如果您仍然觉得占用过高，请避免开启以下选项（一般对性能影响不大）：
						</p>
						<ul>
							<li>流体背景</li>
							<li>歌词模糊</li>
				...
				isHijackDisabled ? (
					<>
						...
						<p>
							您也可以选择跳过。如果出现问题需要修复，可以在插件设置中调出此页面。
						</p>
					</>
				) : (
					<>
						<p className="pass">
							🎉 您的 Refined Now Playing 已经可以正常工作了。
						</p>
						<p>
							点击下方按钮关闭本引导。如果需要，您可以随时可以在插件设置中调出此页面。
						</p>
						<p>
...
export function compatibilityWizard(force = false) {
	...
	const wizardId = "refined-now-playing-wizard";
	let wizard = document.getElementById(wizardId);
	...
	return (
		<div className="hijack-failure-notice">
			<div className="info">
				<div>Hijack 错误</div>
				<div>
					Refined Now Playing 无法正常工作，可能导致歌词无法显示。
					<strong>请重启网易云以修复此问题。</strong>
				</div>
			</div>
...
export async function hijackFailureNoticeCheck() {
	try {
		const succeededHijacks = await betterncm.app.getSucceededHijacks();
		if (
			succeededHijacks.filter((x: string) => x.includes("RefinedNowPlaying"))
				.length > 0
		) {
			return;
		}

		const noticeId = "refined-now-playing-hijack-failure-notice";
		...
		console.warn("[RNP] Hijack check failed", e);
	}
}
